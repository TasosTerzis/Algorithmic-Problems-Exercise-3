...
int main(int argc, char **argv) {
    ...
    // Create the Hash object, to represent the hash object. Pass the dataset in order to hash the images, and the parameters L, w, k
    Hash *input_hash = new Hash(*input_set, L, W_LSH, k);
    std::cout << "Added dataset to Hash" << std::endl;
    ...
    while (running_flag) {
        ...
        for (int i = 0; i < query_set->getNumImages(); i++) {
            ...
            std::map<double, Image *> nearest_neighbours = kNearestNeighbour(*input_hash, *query_image, N);
            ...
            std::map<double, Image *> real_nearest_neighbours = bruteForceLsh(*input_set, *query_image, N);
            ...
            while ((it1 != nearest_neighbours.end()) && (it2 != real_nearest_neighbours.end())) {
                ...
                output << "distanceLSH: " << it1->first << std::endl;
